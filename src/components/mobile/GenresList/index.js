...
import { PageNavBar } from '../HomeNavBar';
    ...
    <>

      {
        view ? (
          ...
          <ModalBox>
            <h3 onClick={() => changeGenre('Todos los géneros', 0)}>Todos los géneros</h3>
            {
              info && info.map((i) => ((i.name !== 'News' && i.name !== 'Soap') && (i.name !== 'Talk')) && (<h3 onClick={() => changeGenre(i.name, i.id)}>{i.name}</h3>))
            }
      ...
      <AbsoluteBox>
        ...
        <PageNavBar title={type === 'movie' ? 'Películas' : 'Series'} />
        {select ? (
          <DataDiv>
